...
function CreatorWithdrawal({ mint, position_id}: { 
  ...
  position_id: number;
    ...
    const handleCreatorWithdrawal = async () => {
        ...
        const res = await axios.post(`${apiBase}/yield/creator-yield`, {
        ...
        if (!data.tx) {
            showToast('Did not return a transaction.', 'error');
        }
  ...
  return (
    <div>
      <button onClick={handleCreatorWithdrawal} disabled={loading} className={loading ? 'loading-button' : 'dashboard-atn-btn'}>
