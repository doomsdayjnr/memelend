...
import ClaimPresaleFees from './ClaimPresaleFees';
...
function PresaleHoldings() {
    ...
    const fetchPositions = async () => {
        try {
        const res = await fetch(`${apiBase}/positions/presale-positions/${publicKey.toString()}`);
        if (!res.ok) {
    ...
    <div className="dashboard-table-frame">
      {positions.length === 0 ? (
        <div className="table-empty-state">
          ...
          <Link to="/presale" className="empty-state-create-btn">
            ➕ Buy your first presale token
          </Link>
        </div>
      ) : (
                ...
                <tr key={position.positionId}>
                  ...
                  <td>
                    {prices[position.mint] ? (
                      ...
                      <span className="status-badge status-closed">Presale Stage</span>
                    )}
                  </td>
                  <td>
                  ...
                  <td>
                    $ {formatLargeNumber(position.pendingRewards)}
                  </td>
                  <td>
                    <span
                  ...
                  <td>
                    <ClaimPresaleFees mint={position.mint}/>
                  </td>
                  <td>
                    {position.openTxSig ? (
                      <a
...
export default PresaleHoldings